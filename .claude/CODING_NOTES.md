...
**A `row_dimensions[row].hidden = True` doesn't survive Excel reapplying the sheet's own AutoFilter — the filter's checked criteria decides visibility if that ever happens, not the stored hidden flag.** (CodeRabbit, PR #9) A row manually hidden (e.g. the footnote) must have its value left *out* of the `Filters(filter=[...])` checked list, or Excel can re-show it on reapply despite the explicit hide. Conversely, a row that must stay *visible* despite a blank column value (e.g. the TOTALS marker) needs `Filters(blank=True)`, or Excel's filter has no criterion matching blank and can hide it instead. Get both directions right, not just one.

**`openpyxl.load_workbook()` cannot read the legacy `.xls` binary format at all — a file-format check belongs at every entry point that accepts a path, not just the UI layer.** (CodeRabbit, PR #9) The JobBOSS tab's file dialog and drop handler advertised `.xls` support copy-pasted from the Fix & Export tab (which genuinely supports it via pandas/xlrd), so a real `.xls` export was accepted by the UI and only failed deep inside `strip()` with a confusing error. Reject non-`.xlsx` input immediately with a clear message, both in the UI and defensively inside the handler itself (which is also directly unit-tested, bypassing the UI gate).
